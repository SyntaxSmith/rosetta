...
export interface RunConversationInput {
  prompt: string;
  /** ChatGPT server-side slug, e.g. "gpt-6-pro". */
  model: string;
  /**
   * Optional `thinking_effort` for the composer's "能力" slider. Captured
   * values (2026-09 picker): "standard" | "extended" | "max" — the 中/高/极高
   * lanes pair these with `gpt-5-6-thinking`. When omitted, the effort is
   * auto-aligned to `model`: -pro slugs send their family's UI value
   * (gpt-6-pro → "standard", gpt-5-5-pro → "extended"), everything else drops
   * the field (the backend then applies the model's natural default).
   */
